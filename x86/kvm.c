...
struct kvm_ext kvm_req_ext[] = {{DEFINE_KVM_EXT(KVM_CAP_COALESCED_MMIO)},
...
bool kvm__arch_cpu_supports_vm(void)
{
  ...
  regs = (struct cpuid_regs){.eax = eax_base + 0x01};
  host_cpuid(&regs);
...
void kvm__arch_init(struct kvm *kvm, const char *hugetlbfs_path, u64 ram_size)
{
  ...
  else
  {
    ...
    if (kvm->ram_start != MAP_FAILED)
       ...
       */
      mprotect(kvm->ram_start + KVM_32BIT_GAP_START, KVM_32BIT_GAP_SIZE,
               PROT_NONE);
  }
  if (kvm->ram_start == MAP_FAILED)
    die("out of memory");
...
void kvm__arch_post_copy(struct kvm *kvm, const char *hugetlbfs_path,
                         u64 ram_size, struct pre_copy_context *ctxt)
{
  struct kvm_pit_config pit_config = {
      .flags = 0,
  };
  ...
  /* We've forked at this point, so we may or may not need to copy our memory
   * state */
  if (ram_size < KVM_32BIT_GAP_START)
  {
    kvm->ram_size = ram_size;
  ...
  else
  {
    if (hugetlbfs_path)
    {
      ...
      kvm__fork_trace(
          ctxt,
          "x86 post-copy: remap/copy RAM with 32-bit gap start bytes=%llu",
          (unsigned long long)mapped_size);
      new_ram = mmap_anon_or_hugetlbfs(kvm, hugetlbfs_path,
                                       ram_size + KVM_32BIT_GAP_SIZE);
      memcpy(new_ram, kvm->ram_start, ram_size + KVM_32BIT_GAP_SIZE);
      if (kvm->ram_start != MAP_FAILED)
        /*
         * We mprotect the gap (see kvm__init_ram() for details) PROT_NONE so
         * that if we accidently write to it, we will know.
         */
        mprotect(kvm->ram_start + KVM_32BIT_GAP_START, KVM_32BIT_GAP_SIZE,
                 PROT_NONE);
      kvm__fork_trace(
          ctxt,
          "x86 post-copy: remap/copy RAM with 32-bit gap complete bytes=%llu",
          (unsigned long long)mapped_size);
    }
    else
    {
      kvm__fork_trace(
          ctxt, "x86 post-copy: inherited COW RAM with 32-bit gap bytes=%llu",
...
int kvm__arch_free_firmware(struct kvm *kvm) { return 0; }
